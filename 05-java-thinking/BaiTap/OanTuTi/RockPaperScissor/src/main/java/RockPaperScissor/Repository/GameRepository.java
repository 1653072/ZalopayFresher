...
public interface GameRepository extends JpaRepository<Game, BigInteger> {
	...
	@Query("SELECT NEW RockPaperScissor.Class.StringDoublePair(ga.username, (SELECT count(*) FROM Game g2 JOIN g2.account g2a WHERE g2a.username=ga.username AND g2.gameResult=1)*1.0/COUNT(ga.username) AS winRating) FROM Game g JOIN g.account ga GROUP BY ga.username ORDER BY winRating DESC")
	public List<StringDoublePair> findWinRatingOfUsers(PageRequest pageRequest);
	default List<StringDoublePair> findTop100WinRating() {
